...
export default function MediaCard({ apiData, forceUpdate }) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
        const response = await axios.delete('/api/lightexercise', {
            ...
            <CardActionArea>
              ...
              <CardContent>
                ...
                <Typography variant="body2" color="textSecondary" component="p">
                  ...
                  <Typography variant="h6" color='textPrimary' style={{fontSize: 16, display: 'inline-block'}}>
                    Date:
                  </Typography> {new Date(data.DateTime).toLocaleDateString("en-US")} <br />

                  <Typography variant="h6" color='textPrimary' style={{fontSize: 16, display: 'inline-block'}}>
                    Time:
                  </Typography> {new Date(data.DateTime).toLocaleTimeString("en-US")}
                </Typography>
              </CardContent>
            </CardActionArea>
